...
namespace GraphQL_MongoDB.Repositories.Implementations
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserTokenPayload RenewAccessToken(RenewTokenInput renewTokenInput)
        {
            ...
            if (string.IsNullOrEmpty(renewTokenInput.AccessToken) || string.IsNullOrEmpty(renewTokenInput.RefreshToken))
            {
            ...
            var user = _user.Find(x => x.UserName == userName &&
                                        x.RefreshToken == renewTokenInput.RefreshToken &&
                                        x.RefreshTokenExpiration > DateTime.Now)
                                .FirstOrDefault();
            if (user == null)
            {
                Message = "Invalid Token";
